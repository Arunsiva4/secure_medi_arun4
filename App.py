...
def ttplogin():
    ...
    if request.method == 'POST':
        if request.form['uname'] == 'admin' and request.form['password'] == 'admin':
            ...
            conn = mysql.connector.connect(user='root', password='', host='localhost', database='1SecureObjectdb')
            cur = conn.cursor()
...
def upload():
    if request.method == 'POST':
        ...
        classifierload = tf.keras.models.load_model('Model/lungmodel.h5')
        import numpy as np
...
def HStatus():
    ...
    conn = mysql.connector.connect(user='root', password='', host='localhost', database='1SecureObjectdb')
    cur = conn.cursor()
    cur.execute("SELECT * FROM requesttb where RHCName ='" + session['hname'] + "' And Status='approved' ")
    data1 = cur.fetchall()
